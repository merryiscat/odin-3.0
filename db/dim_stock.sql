...
-- 테마 배열 검색(포함 질의)
create index if not exists idx_dim_stock_themes    on dim_stock using gin (themes);
...
comment on column dim_stock.themes        is '테마 배열. 뉴스/커뮤니티 등에서 채움(나중 단계).';
