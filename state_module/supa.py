"""
오딘 3.0 state_module — Supabase REST 얇은 도우미.

모듈끼리 import 금지 규칙 때문에 data_pipeline의 것을 가져다 쓰지 않고 여기 따로 둔다.
...
class Supa:
    ...
    def select(self, table: str, params: dict) -> list[dict]:
        """PostgREST 조회. params 예: {"select": "*", "market": "eq.KR-KOSPI", "order": "date"}.
        1,000행 넘으면 offset으로 끝까지 읽는다(order를 꼭 줄 것 — 없으면 페이지가 겹칠 수 있다)."""
        out, offset = [], 0
        while True:
    ...
    def insert(self, table: str, rows: list[dict]) -> None:
        """append 전용 표에 추가(덮어쓰기 없음)."""
        if not rows:
            return
        h = dict(self.headers, Prefer="return=minimal")
        r = self.client.post(f"{self.url}/rest/v1/{table}", headers=h, content=json.dumps(rows, ensure_ascii=False))
